...
#define TIMER_CHECKEXPIRE	600000
...
/*
	...
	if(pRec->Execute(szQuery))
	{
		while(pRec->Fetch())
		{
			...
			sprintf(szQuery, "SELECT * FROM TBL_SELECTGATEIPS WHERE FLD_NAME = N'%s' AND FLD_GAMETYPE='%s'", pServerInfo->szName, m_szGameType);
#ifdef _DEBUG
	GetApp()->SetLog(0,"[SQL QUERY] %s", szQuery);
#endif

...
CGameServer::CGameServer( SOCKET sdClient )
{
	...
	//m_pBuffer = new CCircularBuffer(IOCP_MAXBUF * 2);
}
...
//	int					size=0;
	...
	fnMakeDefMessage( &defMsg, (WORD)RetCmdNum, result, 0, 0, 0 );
...
//	DWORD dwStart = GetTickCount();
	...
	// DB 검사하는 루틴 
	if ( !GetDBServer()->IsRegisteredUserId( lhuman.szID ) )
	{
		return false;
	}
						...
						if (_makesqlparam(szQuery, SQLTYPE_SELECTWHERE, &__ITEMTABLE, lhuman.szName , U_SAVE))
						{
							...
							if ( pRec->Execute( szQuery ) )
							{
								...
								while ( pRec->Fetch() )
								{
									...
									rcd.Block.DBSaveItem.Items[nSaveItem].Index		= (unsigned short)tItemFields.fld_index;
									rcd.Block.DBSaveItem.Items[nSaveItem].Dura		= (unsigned short)tItemFields.fld_dura;
									rcd.Block.DBSaveItem.Items[nSaveItem].DuraMax	= (unsigned short)tItemFields.fld_duramax;
									
									for (int i = 0; i < 14; i++)
										rcd.Block.DBSaveItem.Items[nSaveItem].Desc[i]	= (BYTE)tItemFields.fld_desc[i];

									rcd.Block.DBSaveItem.Items[nSaveItem].ColorR		= (BYTE)tItemFields.fld_colorr;
									rcd.Block.DBSaveItem.Items[nSaveItem].ColorG		= (BYTE)tItemFields.fld_colorg;
									rcd.Block.DBSaveItem.Items[nSaveItem].ColorB		= (BYTE)tItemFields.fld_colorb;
...
//	if (strlen(pszDevide2) - 6 == (int)(sizeof(FDBRecord)*4/3))
	if (strlen(pszDevide2) - 6 == 9260 ) //6594, 6583 , 6582	 // 6647  // Encode FDBRecord size
		fnDecode6BitBuf(pszDevide2, (char *)&rcd, sizeof(FDBRecord));
	else
	{
#ifdef _DEBUG
		char	szLogMsg[128];
		sprintf(szLogMsg, "Packet Size:%d, Struct Size:%d\r\n", strlen(pszDevide2) - 6, (int)(sizeof(FDBRecord)*4/3));
...
bool CGameServer::OnRecv( char *pPacket, int nPacketLen )
{
	...
	GetApp()->SetLog(0,"GAME ID %d", msg.wIdent );
	//
	// 해당 프로토콜 함수를 호출한다.
	//
...
{
	//m_pBuffer->PutData(m_olRecv.szBuf, m_olRecv.nBufLen);

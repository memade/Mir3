...
void _setrecordTHuman(LPTCHARACTERFIELDS lpCharFields, LPTHuman lptHuman)
{
	...
	lptHuman->CX	= (WORD)lpCharFields->fld_cx;
	lptHuman->CY	= (WORD)lpCharFields->fld_cy;
	lptHuman->Dir	= (BYTE)lpCharFields->fld_dir;
	lptHuman->Hair	= (BYTE)lpCharFields->fld_hair;
	lptHuman->Sex	= (BYTE)lpCharFields->fld_sex;
	lptHuman->Job	= (BYTE)lpCharFields->fld_job;
	lptHuman->Gold	= lpCharFields->fld_gold;
	
	lptHuman->HairColorR	= (BYTE)lpCharFields->fld_haircolorr;
	lptHuman->HairColorG	= (BYTE)lpCharFields->fld_haircolorg;
	lptHuman->HairColorB	= (BYTE)lpCharFields->fld_haircolorb;

	// TO PDS:
	//lptHuman->Abil.Level = lpCharFields->fld_level;
	lptHuman->Abil_Level	= (BYTE)lpCharFields->fld_level;
	lptHuman->Abil_HP		= (WORD)lpCharFields->fld_hp;
	lptHuman->Abil_MP		= (WORD)lpCharFields->fld_mp;
	lptHuman->Abil_EXP		= lpCharFields->fld_exp;

	strcpy(lptHuman->HomeMap, lpCharFields->fld_homemap);

	lptHuman->HomeX				= (WORD)lpCharFields->fld_homex;
	lptHuman->HomeY				= (WORD)lpCharFields->fld_homey;

	lptHuman->PkPoint			= lpCharFields->fld_pkpoint;
	lptHuman->AllowParty		= (BYTE)lpCharFields->fld_allowparty;
	lptHuman->FreeGuiltyCount	= (BYTE)lpCharFields->fld_fregulitycount;
	lptHuman->AttackMode		= (BYTE)lpCharFields->fld_attackmode;
	lptHuman->IncHealth			= (BYTE)lpCharFields->fld_inchealth;
	lptHuman->IncSpell			= (BYTE)lpCharFields->fld_incspell;
	lptHuman->IncHealing		= (BYTE)lpCharFields->fld_inchealing;
	lptHuman->FightZoneDie		= (BYTE)lpCharFields->fld_fightzonedie;

	strcpy(lptHuman->UserID, lpCharFields->fld_userid);

	lptHuman->DBVersion				= (BYTE)lpCharFields->fld_dbversion;
	lptHuman->BonusApply			= (BYTE)lpCharFields->fld_bonusapply;
	lptHuman->BonusPoint			= lpCharFields->fld_bonuspoint;
	lptHuman->HorseRide				= (BYTE)lpCharFields->fld_hungrystate;
	lptHuman->DailyQuest			= lpCharFields->fld_testserverresetcount;
	lptHuman->CGHIUseTime			= (WORD)lpCharFields->fld_cghusetime;
	...
	lptHuman->HorseRace		= (BYTE)lpCharFields->fld_horserace;
}
...
void _setrecordTBagItem(LPTITEMFIELDS lpItemFields, LPTBagItem lptBagItem, int *pnBagItem)
{
	...
	lpUserItem->Index		= (WORD)lpItemFields->fld_index;
	lpUserItem->Dura		= (WORD)lpItemFields->fld_dura;
	lpUserItem->DuraMax		= (WORD)lpItemFields->fld_duramax;
	for (int i = 0; i < 14; i++)
		lpUserItem->Desc[i]	= (BYTE)lpItemFields->fld_desc[i];
	lpUserItem->ColorR		= (BYTE)lpItemFields->fld_colorr;
	lpUserItem->ColorG		= (BYTE)lpItemFields->fld_colorg;
	lpUserItem->ColorB		= (BYTE)lpItemFields->fld_colorb;
...
{
	lptAbility->Level		= (BYTE)lpAbilFields->fld_level;
	                      
	lptAbility->AC			= (WORD)lpAbilFields->fld_ac;
	lptAbility->DC			= (WORD)lpAbilFields->fld_dc;
	                      
	lptAbility->HP			= (WORD)lpAbilFields->fld_hp;
	lptAbility->MP			= (WORD)lpAbilFields->fld_mp;
	                      
	lptAbility->MaxHP		= (WORD)lpAbilFields->fld_maxhp;
	lptAbility->MAXMP		= (WORD)lpAbilFields->fld_maxmp;
	                      
	lptAbility->Exp			= lpAbilFields->fld_exp;
	lptAbility->MaxExp		= lpAbilFields->fld_maxexp;
	                      
	lptAbility->Weight		= (WORD)lpAbilFields->fld_weight;
	lptAbility->MaxWeight	= (WORD)lpAbilFields->fld_maxweight;

	lptAbility->WearWeight		= (BYTE)lpAbilFields->fld_wearweight;
	lptAbility->MaxWearWeight	= (BYTE)lpAbilFields->fld_maxwearweight;
	lptAbility->HandWeight		= (BYTE)lpAbilFields->fld_handweight;
	lptAbility->MaxHandWeight	= (BYTE)lpAbilFields->fld_maxhandweight;
										  
	lptAbility->ATOM_MC[ATOM_FIRE]		= (WORD)lpAbilFields->fld_atomfire_mc;
	lptAbility->ATOM_MC[ATOM_ICE]		= (WORD)lpAbilFields->fld_atomice_mc;
	lptAbility->ATOM_MC[ATOM_LIGHT]		= (WORD)lpAbilFields->fld_atomlight_mc;
	lptAbility->ATOM_MC[ATOM_WIND]		= (WORD)lpAbilFields->fld_atomwind_mc;
	lptAbility->ATOM_MC[ATOM_HOLY]		= (WORD)lpAbilFields->fld_atomholy_mc;
	lptAbility->ATOM_MC[ATOM_DARK]		= (WORD)lpAbilFields->fld_atomdark_mc;
	lptAbility->ATOM_MC[ATOM_PHANTOM]	= (WORD)lpAbilFields->fld_atomphantom_mc;
	lptAbility->ATOM_MAC[ATOM_FIRE]		= (WORD)lpAbilFields->fld_atomfire_mac;
	lptAbility->ATOM_MAC[ATOM_ICE]		= (WORD)lpAbilFields->fld_atomice_mac;
	lptAbility->ATOM_MAC[ATOM_LIGHT]	= (WORD)lpAbilFields->fld_atomlight_mac;
	lptAbility->ATOM_MAC[ATOM_WIND]		= (WORD)lpAbilFields->fld_atomwind_mac;
	lptAbility->ATOM_MAC[ATOM_HOLY]		= (WORD)lpAbilFields->fld_atomholy_mac;
	lptAbility->ATOM_MAC[ATOM_DARK]		= (WORD)lpAbilFields->fld_atomdark_mac;
	lptAbility->ATOM_MAC[ATOM_PHANTOM]	= (WORD)lpAbilFields->fld_atomphantom_mac;
/*	lptAbility->FameLevel;				  
	lptAbility->MiningLevel;			  
	lptAbility->FramingLevel;
	lptAbility->FishingLevel;
...
{
	lptUseMagicInfo->MagicId	= (WORD)lpMagicFields->fld_magicid;
	lptUseMagicInfo->Level		= (BYTE)lpMagicFields->fld_level;
	lptUseMagicInfo->Key		= (char)lpMagicFields->fld_key;

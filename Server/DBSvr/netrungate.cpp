 ...
 char* pData)
{
 ...
 fnMakeDefMessage(&defMsg, (WORD)nPacketID, nRecog, (WORD)nParam, (WORD)nTag, (WORD)nSeries);
...
bool CRunGate::OnQueryChr(sGateUserInfo* pUser, char* pBody)
{
 ...
 for (unsigned int i = 0; i < strlen(backid) - 1; i++)
 {
  switch (backid[i])
  {
  case '\'':
...
bool CRunGate::OnNewChr(sGateUserInfo* pUser, char* pBody)
{
 ...
 while (*pszName)
 {
  // all 2Byte character cacel..
  if (*pszName > 127) error = 0;
...
bool CRunGate::OnSelChr(sGateUserInfo* pUser, char* pBody)
{
 ...
 bstr szID, szName, szCert;
 _pickstring(pBody, '/', 0, &szID);
 _pickstring(pBody, '/', 1, &szName);
 //	_pickstring( pBody, '/', 2, &szCert );	
 ...
 //	int nCert = atoi( szCert ) ^ 0x25374963;
 //	if ( !GetDBServer()->IsRegisteredAdmission( (char * )szID, nCert ) )
 //	{
 //		SendResponse( pUser->nHandle, SM_OUTOFCONNECTION );
 //		return true;
 //	}
 ...
 if (fOK)
 {
  ...
  // 마지막 선택한 넘 
  strcpy(pUser->szLastSelChar, (char*)szName);
...
BOOL IsHangul(CHAR* pszChar)
{
 ...
 if (cChar & 0x800)
 {
  wchDBCS = (WORD)((cChar << 8) + cNextChar);
  if (wchDBCS > 0xCAA0)
  {
   // 한자.
   return FALSE;

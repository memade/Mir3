...
bool CIocpObject::Recv()
{
	...
	//memset( m_olRecv.szBuf, 0, IOCP_MAXBUF );
	//m_olRecv.wsaBuf.len = IOCP_MAXBUF;
	//m_olRecv.wsaBuf.buf = m_olRecv.szBuf;
	...
	while ( true )
	{
		...
		for ( pNode = m_pListProcessing->GetHead(); pNode; pNode = pNode->GetNext() )
		{
			...
			while ( pObject->ExtractPacket( szPacket, &nPacketLen ) )
			{
				...
				{
					OutputDebugString(pObject->IP());
					OutputDebugString(" IOCP Closed by Packet Error\n");
					pObject->m_bClosed = true;
					break;
				}
			}
			...
			{
				OutputDebugString(pObject->IP());
				OutputDebugString(" IOCP Closed by Buffer Overrun\n");
				pObject->m_bClosed = true;
			}
...
void CIocpHandler::__tcWorker()
{
	...
	while ( true )
	{
		...
		{
			OutputDebugString(pObject->IP());
			OutputDebugString(" IOCP Closed by Normal\n");
			pObject->m_bClosed = true;
		}
...
{
//	static FILE *fp = fopen( "C:\\RecvDirect.log", "wb" );
//	fprintf( fp, "[%s:%d] %d Bytes 수신되었습니다.\r\n", pObject->IP(), pObject->Port(), nTransferred );
//	fflush( fp );
